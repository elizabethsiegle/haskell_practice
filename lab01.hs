...
greet2 q = if null q then "Hi there" else "Hi " ++ q
...
to a |String|.
do something like countDown 0 = "Too low"
then handle the case where the number is not zero
like: countDown 0 = 0
countDown x = ...
...
sumDigits:: Int -> Int
sumDigits 0 = 0
sumDigits d =(d `mod` 10) + sumDigits (d `div` 10)
...
getFactors:: Int -> [Int]
getFactors p1 = [x | x <- [1..p1], p1 `mod` x == 0]
isPrime:: Int -> Bool
isPrime 0 = False
isPrime 1 = False 
isPrime pp = getFactors pp == [1, pp]
